...
def runAccessAVLApp(dataset):
    with open("data/oklist1.txt","r") as file:
        for x in file:
            aId = str(x.strip().split(" ")[0])
            os.system("java -cp bin  AccessAVLApp "+aId)
...
for n in range(1,11):
    ...
    aFile = open("data/oklist1.txt", "w")
    for i in range(dataset):
        aFile.write(file2[aList1[i]])
    aFile.close()
    runAccessAVLApp(dataset)
